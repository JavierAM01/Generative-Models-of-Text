...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def __init__(self, d: int, base: int = 10_000):
        ...
        theta = torch.tensor([base**(-2*(i-1)/d) for i in range(1, d//2 + 1)])
        self.theta = torch.concat([theta, theta], axis=0)
        self.C = None
    ...
    def _build_cache(self, Y: torch.Tensor):
        ...
        # avoid rebuilding part of the cache
        if self.C is not None:  
            if self.C.shape[0] > N:
                self.C = self.C[:N]
            else: 
                C2 =  self.theta * torch.arange(self.C.shape[0],N).reshape((-1,1))
                self.C = torch.concat([self.C, C2], axis=0)
        # create the cache from scratch
        else:
            self.C = self.theta * torch.arange(0,N).reshape((-1,1))
    ...
    def forward(self, Y: torch.Tensor):
        ...
        if self.C is None or self.C.shape[0] != Y.shape[0]: 
            self._build_cache(Y)

        sin = torch.sin(self.C).unsqueeze(0).unsqueeze(0).to(Y.device)
        #sin = torch.repeat_interleave(sin, 2, dim=-1)
        cos = torch.cos(self.C).unsqueeze(0).unsqueeze(0).to(Y.device)
        #cos = torch.repeat_interleave(cos, 2, dim=-1)

        d = self.d

        Y1, Y2 = Y[:,:,:, :d//2], Y[:,:,:, d//2:]
        Ytilde = Y * cos + torch.concat([-Y2, Y1], axis=3) * sin
...
class GroupedQueryAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.head_ratio = config.n_query_head // config.n_kv_head
        ...
        self.k_proj = nn.Linear(config.n_embd, config.n_embd // self.head_ratio) # Do NOT rename k_proj.
        self.v_proj = nn.Linear(config.n_embd, config.n_embd // self.head_ratio) # Do NOT rename v_proj.
        ...
        self.rope = config.rope
        if self.rope:
            self.query_rotary_pe = RotaryPositionalEmbeddings(d=self.n_embd // self.n_head) # Do NOT rename.
            self.key_rotary_pe = RotaryPositionalEmbeddings(d=self.n_embd // self.n_head) # Do NOT rename.
    ...
    def forward(self, x):
        ...
        k = self.k_proj(x)  # (b, t, n_embd / head_ratio)
        v = self.v_proj(x)  # (b, t, n_embd / head_ratio)

        # split the embedding dimension (n_embd) across the number of heads by introducing an additional 'h' dimension
        # reshape the query, key, value tensors to increase efficiency of matrix multiplication
        # b = batch size, t = sequence length, h = number of heads, d = n_embd / number of heads

        q = rearrange(q, "b t (h d) -> b h t d", h=self.n_head)     # (b, n_head, t, d)
        k = rearrange(k, "b t (h d) -> b h t d", h=self.n_kv_head)  # (b, n_kv_head, t, d_kv)
        v = rearrange(v, "b t (h d) -> b h t d", h=self.n_kv_head)  # (b, n_kv_head, t, d_kv)

        if self.rope:
            q = self.query_rotary_pe(q)
            k = self.key_rotary_pe(k)

        # Repeat K and V for multiple Query heads per KV head
        k = repeat(k, "b h t d -> b (h r) t d", r=self.head_ratio)  # Expand KV heads
        v = repeat(v, "b h t d -> b (h r) t d", r=self.head_ratio)
